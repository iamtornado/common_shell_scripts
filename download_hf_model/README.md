...
### Q: 下载中断后如何恢复？
A: 
- 脚本支持断点续传（由底层的 `huggingface_hub` 自动处理）
- 重新运行相同命令即可，已下载的文件不会重复下载
- **注意**：断点续传时，输出中不会明确显示"正在续传"的提示，但 `hf download` 命令会自动跳过已缓存的文件，只下载缺失的部分。如果大部分文件已下载，下载会很快完成，这是正常的续传行为。
